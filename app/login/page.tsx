...
function AuthForm() {
  ...
  return <section className="auth-card"><div className="auth-intro"><Brand/><span className="eyebrow">APNA PAAN. APNI PASAND.</span><h1>{mode === "login" ? "Welcome back" : "Join MeraPaan"}</h1><p>{mode === "login" ? "Sign in to manage your pickups and favourites." : "Order as a customer or bring your Hyderabad paan shop online."}</p></div>
    ...
    <form className="form" onSubmit={submit}>
      <div className="role-options" aria-label="Account type"><button type="button" className={role === "customer" ? "active" : ""} onClick={() => setRole("customer")}><Icon name="user"/><span><b>Customer</b><small>Order for pickup</small></span></button><button type="button" className={role === "seller" ? "active" : ""} onClick={() => setRole("seller")}><Icon name="shop"/><span><b>Seller</b><small>Open seller dashboard</small></span></button></div>
      {mode === "register" ? <><label>Full name<input required value={name} onChange={(event) => setName(event.target.value)} autoComplete="name" placeholder="Your full name"/></label><label>Mobile number<input required value={mobile} onChange={(event) => setMobile(event.target.value)} inputMode="tel" autoComplete="tel" placeholder="10-digit mobile number" pattern="[0-9 +()-]{8,16}"/></label>{role === "seller" ? <label>Shop name<input required value={storeName} onChange={(event) => setStoreName(event.target.value)} placeholder="Your paan shop name"/></label> : null}</> : null}
      ...
      {message ? <p className={message.includes("sent") ? "form-message" : "form-error"} role="alert">{message}</p> : null}<button className="button button-primary full" disabled={busy}>{busy ? "Please wait…" : mode === "login" ? `Sign in as ${role}` : role === "seller" ? "Create seller account" : "Create customer account"}<Icon name="arrow"/></button>
